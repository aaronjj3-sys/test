...
export async function sourceDoorsFromApollo(input) {
  ...
  if (userFilters.companies?.length && !filters.organization_ids) {
    /* bare company names go in as keywords (domain-less; Organization Search costs credits) */
    filters.q_organization_domains_list = userFilters.companies
      .filter((c) => /\./.test(c))
      .concat(filters.q_organization_domains_list || []);
  }
  const keywordTerms = [
  ...
  const perPage = Math.min(MAX_PEOPLE_SEARCH_RESULTS, 100);
  for (let page = 1; people.length < limit && page <= 5; page++) {
    const search = await peopleSearch({ ...filters, page, per_page: perPage });
