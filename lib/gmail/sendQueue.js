...
import { getGoogleConnection, sendEmail, getThread, createDraft } from "./client.js";
...
function openTrackingUrl(userId, messageId) {
  ...
  return base && messageId ? `${base}/api/track/open?m=${encodeURIComponent(messageId)}&u=${encodeURIComponent(userId)}` : null;
}
...
export async function syncUser(userId) {
  ...
  for (const row of due) {
    ...
    try {
      const sent = await sendEmail({
        ...
        subject: row.subject,
        body: row.body,
        attachments: profileRow.send_prefs?.attachResume && profile.resumeAttachment ? [profile.resumeAttachment] : [],
  ...
  for (const row of toCheck) {
    ...
    if (!latestInbound) {
      if (row.status === "opened") updates.push({ messageId: row.id, doorId: door.id, status: "opened" });
      continue;
    }
