...
async function gmailFetch(token, path, options = {}) {
  ...
  try {
    ...
    if (err?.name === "AbortError") throw new Error("Gmail request timed out");
    throw err;
  } finally {
    clearTimeout(timer);
  }
...
function extractBody(payload) {
  return extractBodyParts(payload).body;
}
...
function extractBodyParts(payload) {
  ...
  while (stack.length) {
    ...
    if (!part) continue;
    if (part.mimeType === "text/plain" && part.body?.data && plain === null) plain = decodeBody(part.body.data);
  ...
  const body = plain ? stripQuotedText(plain) : html ? stripQuotedText(stripHtml(html)) : "";
  return { body, bodyHtml: html || "" };
}

function normalizeMessage(message, myEmail = "") {
  ...
  return {
    ...
    date: internal
      ...
      : headerValue(payload, "Date") || null,
    internalDate: internal || null,
    subject: headerValue(payload, "Subject"),
    body: parts.body,
    bodyHtml: parts.bodyHtml,
    attachments: extractAttachments(payload, message.id),
    messageIdHeader: headerValue(payload, "Message-ID") || headerValue(payload, "Message-Id"),
    isFromMe: Boolean(myEmail) && from.toLowerCase().includes(myEmail.toLowerCase()),
  };
}

export async function searchMessages(userId, query, maxResults = 100) {
  const connection = await getGoogleConnection(userId);
  if (!connection) throw new Error("google_not_connected");
  const token = await getAccessToken(connection);
  const cap = Math.min(Math.max(Number(maxResults) || 1, 1), 100);
  const messages = [];
  let pageToken = "";
  ...
  while (messages.length < cap) {
    const params = new URLSearchParams({
      q: query,
      maxResults: String(Math.min(50, cap - messages.length)),
    });
...
export async function getMessage(userId, gmailMessageId) {
  const connection = await getGoogleConnection(userId);
  if (!connection) throw new Error("google_not_connected");
  const token = await getAccessToken(connection);
  const data = await gmailFetch(token, `messages/${encodeURIComponent(gmailMessageId)}?format=full`);
  return normalizeMessage(data, connection.provider_email || "");
}
...
export async function getThread(userId, threadId) {
  ...
  const messages = (data.messages || []).map((m) => normalizeMessage(m, myEmail));
