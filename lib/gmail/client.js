...
/* base64 bodies must wrap at 76 chars per RFC 2045 */
...
function buildRawMessage({ from, to, toName, subject, body, inReplyTo, references, attachments = [] }) {
  const lines = [
    `From: ${from}`,
  ...
  /* multipart/mixed: text part first, then one part per attachment */
  const boundary = `knock_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
  ...
  const parts = [
    ...
    ...attachments.map((a) => {
      ...
      return [
        `--${boundary}`,
        `Content-Type: ${a.mime || "application/octet-stream"}; name="${filename}"`,
        `Content-Disposition: attachment; filename="${filename}"`,
        "Content-Transfer-Encoding: base64",
        "",
        wrapBase64(a.dataBase64 || ""),
      ].join("\r\n");
    }),
    `--${boundary}--`,
  ];
...
export async function sendEmail({ userId, to, toName, subject, body, threadId, inReplyTo, references, attachments }) {
