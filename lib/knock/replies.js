...
/** Classify the recipient's latest message in a thread.
    → { type, wantsCall, sentiment, summary, scheduling } or null. */
export async function classifyReply({ profile, door, threadMessages }) {
  ...
  return {
    ...
    summary: out.summary || "",
    scheduling: out.scheduling || { intent: "none", timeText: "", dateTime: "" },
  };
}

/** Draft the auto-reply to a classified response.
    meetLink: string URL or { url, when }. → { subject, body } or null. */
export async function draftReply({ profile, door, threadMessages, classification, tone, styleProfile, meetLink, availabilityOptions, calendarEvent }) {
  if (!openaiConfigured() || !Array.isArray(threadMessages) || !threadMessages.length) return null;
  const { system, prompt } = replyDraftPrompt({ profile, door, threadMessages, classification, tone, styleProfile, meetLink, availabilityOptions, calendarEvent });
  const out = await openaiJSON({
    system,
    prompt,
