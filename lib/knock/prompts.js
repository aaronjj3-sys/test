...
function styleBlock(styleProfile) {
  ...
  if (styleProfile.openingStyle) lines.push(`- Opening style: ${clip(styleProfile.openingStyle, 120)}.`);
  if (styleProfile.closingStyle) lines.push(`- Closing style: ${clip(styleProfile.closingStyle, 120)}.`);
  ...
  if (styleProfile.commonPhrases?.length) {
    lines.push(`- Common phrases to echo sparingly: ${styleProfile.commonPhrases.slice(0, 8).map((p) => clip(p, 60)).join("; ")}.`);
  }
  if (styleProfile.avoid?.length) {
    lines.push(`- Avoid: ${styleProfile.avoid.slice(0, 6).map((p) => clip(p, 80)).join("; ")}.`);
  }
  if (styleProfile.exampleRules?.length) {
    lines.push(`- Style rules: ${styleProfile.exampleRules.slice(0, 6).map((p) => clip(p, 90)).join("; ")}.`);
  }
