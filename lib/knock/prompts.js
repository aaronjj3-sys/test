  ...
  properties: {
    ...
    education: {
      ...
      items: {
        ...
        additionalProperties: false,
        properties: {
          school: { type: "string", description: "Full school name as written" },
          degree: { type: "string", description: "Degree and major, e.g. B.A. Business Administration" },
          when: { type: "string", description: "Years attended or expected graduation, e.g. 2023 - 2027" },
          bullets: { type: "array", items: { type: "string" }, description: "GPA, honors, relevant coursework, max 3" },
        },
        required: ["school", "degree", "when", "bullets"],
      },
      description: "Every education entry on the resume, most recent first",
    },
    sections: {
      type: "array",
      items: {
...
export const REPLY_CLASSIFICATION_SCHEMA = {
  ...
  required: ["type", "wantsCall", "sentiment", "summary"],
};
...
/* Few-shot voice replication: real emails the user edited or wrote in-app
   (profile.editedSamples, newest first) plus raw writingSamples excerpts.
   Total budget ~2400 chars across at most 3 examples; strings shorter than
   40 chars are skipped (defends against file names standing in for text). */
const VOICE_SAMPLE_TOTAL_CHARS = 2400;
const VOICE_SAMPLE_MIN_CHARS = 40;

function pickVoiceSamples(profile = {}) {
  const usable = (arr) => (Array.isArray(arr) ? arr : [])
    .filter((s) => typeof s === "string" && s.trim().length >= VOICE_SAMPLE_MIN_CHARS)
    .map((s) => s.trim());
  const picks = [...usable(profile.editedSamples), ...usable(profile.writingSamples)].slice(0, 3);
  const out = [];
  let budget = VOICE_SAMPLE_TOTAL_CHARS;
  for (const s of picks) {
    if (budget < 200) break;
    const t = clip(s, Math.min(1200, budget));
    out.push(t);
    budget -= t.length;
  }
  return out;
...
function voiceSamplesBlock(profile) {
  const samples = pickVoiceSamples(profile);
  ...
  return [
    ...
    ...samples.map((s, i) => `VOICE EXAMPLE ${i + 1}:\n${s}`),
  ].join("\n\n");
}

function scoutSystem(tone, styleProfile, profile) {
  return [SCOUT_SYSTEM, toneBlock(tone), styleBlock(styleProfile), voiceSamplesBlock(profile || {})]
    .filter(Boolean)
    .join("\n\n");
}
...
- Limits: skills max 14 (tools and hard skills only, no soft skills), quantifiedWins max 6 (achievements that contain a number), experience max 6 entries most recent first with max 3 bullets each, sections max 8 with max 8 items each.
...
Return JSON with "subject" and "body".`;
  return { system: scoutSystem(tone, styleProfile, profile), prompt };
}
...
export function classifyReplyPrompt({ door = {}, threadMessages = [] } = {}) {
...
Return JSON with "type", "wantsCall", "sentiment", and "summary".`;
  return { system, prompt };
}

/** Auto-reply to a classified response. */
export function replyDraftPrompt({ profile = {}, door = {}, threadMessages = [], classification = {}, tone, styleProfile, meetLink } = {}) {
  const signoff = signoffOf(profile);
  ...
  const meetBlock = link?.url && classification?.wantsCall
...
Guidance for this reply type: ${typeGuide}${meetBlock}

Rules:
- Under 90 words, plain text. Respond to what they actually wrote; never re-pitch the first email, they already read it.
- One goal only. If there is an ask, it is concrete and sits in its own short line before the signoff, phrased with gratitude, never as a demand.
