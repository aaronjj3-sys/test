...
export function mockSourcing(input = {}) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const [title, seniority] = titles[i % titles.length];
    const [companyName, companyDomain, location] = COMPANIES[(i * 3) % COMPANIES.length];
    pool.push({
  ...
  const terms = [...(userFilters.keywords || []), ...(userFilters.companies || [])].map((t) => t.toLowerCase());
  if (terms.length) {
  ...
  const ranked = rankDoors(pool, profile, { seniorities: ["founder", "owner", "c_suite", "partner", "vp", "head", "director", "manager"] }).slice(0, limit);
